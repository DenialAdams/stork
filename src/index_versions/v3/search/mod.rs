    ...
    let mut output_results: Vec<OutputResult> = excerpts_by_index
        ...
        .map(|(entry_index, ies)| {
            let data = EntryAndIntermediateExcerpts {
                ...
                intermediate_excerpts: ies,
            };
            OutputResult::from(data)
        })
        .collect();
    // We can't use sort_by_key without incurring an undesired clone
    // https://stackoverflow.com/q/47121985
    output_results.sort_unstable_by(|x , y | {
        (std::cmp::Reverse(x.score), x.entry.title.as_str()).cmp(&(std::cmp::Reverse(y.score), y.entry.title.as_str()))
    });
